...
def get_db_config_data():
    _config = {
        ...
        "password": os.getenv("PASSWORD") if os.getenv("PASSWORD") else "",
        "database": os.getenv("DATABASE") if os.getenv("DATABASE") else "test1",
...
def generate_database(config):
    ...
    print(Style.BRIGHT + Fore.RED + f"This is database: {db_name}")
